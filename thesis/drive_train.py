...
def perform_hdf5_conversion():
    ...
    output_paths.append(_convert_to_hdf5(os.path.join(settings.TRAINING_PATH, settings.FOLDER_IMAGES),
                                         (settings.IMG_HEIGHT, settings.IMG_WIDTH, settings.IMG_CHANNELS_TIF),
                                         exts=".tif"))
    output_paths.append(_convert_to_hdf5(os.path.join(settings.TRAINING_PATH, settings.FOLDER_MANUAL_1),
                                         (settings.IMG_HEIGHT, settings.IMG_WIDTH, settings.IMG_CHANNELS_GIF),
                                         exts=".gif"))
    output_paths.append(_convert_to_hdf5(os.path.join(settings.TRAINING_PATH, settings.FOLDER_MASK),
                                         (settings.IMG_HEIGHT, settings.IMG_WIDTH, settings.IMG_CHANNELS_GIF),
                                         exts=".gif"))

    # Do the same for the test images
    output_paths.append(_convert_to_hdf5(os.path.join(settings.TEST_PATH, settings.FOLDER_IMAGES),
                                         (settings.IMG_HEIGHT, settings.IMG_WIDTH, settings.IMG_CHANNELS_TIF),
                                         exts=".tif"))
    output_paths.append(_convert_to_hdf5(os.path.join(settings.TEST_PATH, settings.FOLDER_MANUAL_1),
                                         (settings.IMG_HEIGHT, settings.IMG_WIDTH, settings.IMG_CHANNELS_GIF),
                                         exts=".gif"))
    output_paths.append(_convert_to_hdf5(os.path.join(settings.TEST_PATH, settings.FOLDER_MASK),
                                         (settings.IMG_HEIGHT, settings.IMG_WIDTH, settings.IMG_CHANNELS_GIF),
                                         exts=".gif"))
...
def convert_img_to_pred(ground_truths, verbose=False):
    ...
    new_masks = np.empty((ground_truths.shape[0], im_h * im_w, 2))
    ...
    new_masks = np.reshape(new_masks, (ground_truths.shape[0], im_h, im_w, 2))
...
if __name__ == "__main__":
    # Convert images to HDF5 format (without applying any preprocessing), only required once
    ...
    patch_ground_truths = convert_img_to_pred(patch_ground_truths, settings.VERBOSE)
